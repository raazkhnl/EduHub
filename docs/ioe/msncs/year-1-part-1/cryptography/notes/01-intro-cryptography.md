...
*The key space of a cryptosystem is the set of all possible keys. The key length, expressed in bits, determines the size of the key space — a $k$-bit key gives $2^k$ possible keys.*
...
**Zero-knowledge.** Whatever the verifier learns from the interaction, the verifier could have produced *without* talking to the prover — meaning the interaction reveals nothing beyond the bare truth of $S$.
